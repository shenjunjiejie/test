...
import com.ucash_test.lirongyunindialoan.annonation.POST;
    ...
    private static void inject(final Activity activity) throws Exception {
        if(activity.getClass().isAnnotationPresent(SetActivity.class)){
            ...
            if(id < 0){
                ...
                if(activity.getClass().isAnnotationPresent(Title.class)){
                    Title title = activity.getClass().getAnnotation(Title.class);
                    activity.setContentView(title.value()[0]);
                    TextView tv_title = activity.findViewById(title.value()[1]);
                    tv_title.setText(title.title());
                    tv_title.setTextColor(title.textColor());
                    ImageView back = activity.findViewById(title.value()[2]);
                    back.setColorFilter(title.backColor());
                    back.setOnClickListener((View view)->{
                        activity.finish();
                    });
                    LinearLayout bg = activity.findViewById(title.value()[3]);
                    bg.setBackgroundColor(title.bgcolor());
                    activity.getLayoutInflater().inflate(id, activity.findViewById(title.value()[4]));
                }
                else{
